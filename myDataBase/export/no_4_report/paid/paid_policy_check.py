...
LEFT JOIN rm_advance_agent_waiting raaw on raaw.mobile=pp.create_account
LEFT JOIN agent_type att on att.agent_type_code=aa.agent_type_code
where  aa.test_account in (0,4) and pp.policy_status not in (-1,101,106,107,109) and pe.id is null  and (p.pay_time_type !=2 OR (p.pay_time_type=2 and pp.pay_status=102))  and  DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR) >='2020-01-01 00:00:00' and  DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR) <'2020-11-01 00:00:00'
union
-- general
SELECT 
...
where  aa.test_account in (0,4) and pp.policy_status not in (-1,101,106,107,109)and  er.id is null and  DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR)>='2020-01-01 00:00:00' and DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR)<'2020-11-01 00:00:00'
...
LEFT JOIN rm_advance_agent_waiting raaw on raaw.mobile=pp.create_account
LEFT JOIN agent_type att on att.agent_type_code=aa.agent_type_code
where  aa.test_account in (0,4) and pp.policy_status not in (-1,101,106,107,109) and DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR)>='2020-01-01 00:00:00' and DATE_ADD(pm.actual_pay_time,INTERVAL 7 HOUR)<'2020-11-01 00:00:00' 
...
## 获取模板
def get_source():
    df_resource = pd.read_excel('paid_template.xlsx')
    return df_resource
...
def generate_sys_info(policy_info_by_codes):
    policy_info_by_codes = policy_info_by_codes.drop(
        ['No','Fuse Code', 'Associated Number', 'fuse_policy_code_map',	'associated_policy_code_map','order_time'], axis=1)
...
write_to_excel(policy_unique,'paid_policy')
policy_duplicate = get_duplicate(policy_info_by_codes)
write_to_excel(policy_duplicate,'paid_policy_duplicate')
data = generate_sys_info(policy_info_by_codes)
write_to_excel(data,'paid_policy_not_in_id')
## 关闭连接
mdbF.shoutdown()
db.shoutdown()
