...
def get_data():
    ...
    data = db.querysql(
        '''
        SELECT fuse_policy_code,insured_name, start_date , end_date ,plate_number,brand, type, series,year,price,car_type from (
SELECT 
...
AND epc.policy_status not in (-1,101)
AND DATE_ADD(IFNULL(epc.ops_oder_time,epc.create_time),INTERVAL 7 hour)>='2019-01-01' 
AND DATE_ADD(IFNULL(epc.ops_oder_time,epc.create_time),INTERVAL 7 hour)<='2020-08-20') tt where insured_name!='' and insured_name is not null
...
def write_to_excel(data_df):
    ...
    table_name = export_path+'Car_BI_detail_policy' + (u'_%d%02d%02d_%02d%02d%02d.xlsx' % (t.year, t.month, t.day,t.hour,t.minute,t.second))
